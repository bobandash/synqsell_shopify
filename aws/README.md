...
```bash
synqsell_lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
synqsell_lambda$ sam local start-api
synqsell_lambda$ curl http://localhost:3000/
```
...
```bash
synqsell_lambda$ sam logs -n HelloWorldFunction --stack-name synqsell_lambda --tail
```
...
```bash
synqsell_lambda$ cd hello-world
...
```bash
sam delete --stack-name synqsell_lambda
```
